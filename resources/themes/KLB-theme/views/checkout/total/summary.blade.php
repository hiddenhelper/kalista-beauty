...
<div class="order-summary fs16">
    ...
    <div>
        ...
        <span style="color: #761c19">({{number_format($cart->base_discount_amount / ($cart->base_sub_total)*100,2)}}%) </span>
        <br>
        <span >Promotion name:</span>
        <span>{{$cart->coupon_code}}</span>
        <br>
        <span >Promotion id:</span>
        <span>{{$cart->applied_cart_rule_ids}}</span>
        <br>
        <span>Dollars Saved: </span>
        <span>     -{{ core()->currency($cart->base_discount_amount) }} </span>
        <br>
