...
class OrderRepository extends Repository
{
    ...
    /**
     * get the payment method related to this orderId
     */
    public function getPaymentMethod($orderid){
        try{
            DB::beginTransaction();
            //first get customer and cart id from cart table
            $paymentMethod = DB::select('SELECT method FROM order_payment WHERE order_id = ?',[$orderid])[0]->method;
            DB::commit();
     ...
     * @return \Webkul\Sales\Contracts\Order
     */
    public function create_withId(array $data, $transactionId)
    {
        DB::beginTransaction();
        ...
        try {
            ...
            $data['status'] = 'pending';
            //add transaction id to data array
    ...
    public function reorder($orderId)
    {
        try {
            DB::beginTransaction();
            //first get customer and cart id from cart table
            $info = DB::select('SELECT cart_id, customer_id FROM orders WHERE id = ?',[$orderId]);
            $customerid = $info[0]->customer_id;
            $cartid = $info[0]->cart_id;
            /**
             * create a temp table and populate the cart information related to the order 
             * remove primary key in temp table
             * insert data from temp table to cart table with fresh primary key
             * get newly inserted id and update the status of cart to active
             * change status of old cart to inactive
             */
            $schemaName = '`'.env('DB_DATABASE').'`';
            Log::debug($schemaName);
            DB::Statement('DROP TEMPORARY TABLE IF EXISTS '.$schemaName.'.tmptable_1');
            DB::Statement('CREATE TEMPORARY TABLE '.$schemaName.'.tmptable_1 SELECT * FROM '.$schemaName.'.cart WHERE id = ?',[$cartid]);
            DB::Statement('UPDATE '.$schemaName.'.tmptable_1 SET id = NULL');
            DB::insert('INSERT INTO '.$schemaName.'.cart SELECT * FROM '.$schemaName.'.tmptable_1;');
            $newid = DB::select('SELECT LAST_INSERT_ID() AS newid')[0]->newid;
            DB::update('UPDATE '.$schemaName.'.cart SET is_active = 0 WHERE customer_id = ?',[$customerid]);
            DB::update('UPDATE '.$schemaName.'.cart SET is_active = 1 WHERE id = ?',[$newid]);
            DB::Statement('DROP TEMPORARY TABLE IF EXISTS '.$schemaName.'.tmptable_1');

            /**
             * cart details section with similar logic
             */
            DB::Statement('DROP TEMPORARY TABLE IF EXISTS '.$schemaName.'.tmptable_2');
            DB::Statement('CREATE TEMPORARY TABLE '.$schemaName.'.tmptable_2 SELECT * FROM '.$schemaName.'.cart_items WHERE cart_id = ?',[$cartid]);
            DB::Statement('UPDATE '.$schemaName.'.tmptable_2 SET id = NULL, cart_id = ?',[$newid]);
            DB::insert('INSERT INTO '.$schemaName.'.cart_items SELECT * FROM '.$schemaName.'.tmptable_2;');
