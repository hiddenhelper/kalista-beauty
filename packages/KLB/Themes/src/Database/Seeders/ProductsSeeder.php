...
class ProductsSeeder extends CSVSeeder
{
    ...
    {
        $this->sortCsv = false;
        $this->csvPath = 'data/productData.csv';
        ...
        /**
     * Go over the entire CSV and set each product as simple product.
     *
     * @return void
     */
    public function seedFromCsv()
    {
        $nonConfigurableProducts = $this->csv;
        Log::debug('Importing ' . $nonConfigurableProducts->count() . ' non-configurable products...');
        $this->seedFromCollection($nonConfigurableProducts);
    }
    /**
     * Take a collection of models and seed them individually.
     *
    ...
    private function seedFromCollection(Collection $collection)
    {
        $this->successes = 0;
        $this->failed = 0;
        ...
        Log::debug("Imported $this->successes products successfully. $this->failed were not imported.");
    }

    /**
     * Create the product models associated with the given csv product.
     *
     * @param array $product
     *
     * @return bool
    ...
    public function setBaseProductDataForModel()
    {
        $this->baseProductData = [
            ...
            'barcode' => str_replace("'", "", $this->product['Barcode']),
            'yotpoid' => str_replace("'", "", $this->product['Barcode']),
        ];
    }
    ...
    {
        $baseProduct = $this->findBaseProductBySku($this->product['Sku ']);
        if ($baseProduct) {
            $baseProduct->delete();
        }
    }
    ...
    public function setAttributeData()
    {
        ...
        $attributeMap = collect([
            ...
            "meta_keywords" => 'Tags/catagories/collections',
            "meta_description" => 'SEO Description',
            "price" => 'Selling Price',
            "cost" => "Cost",
            // "special_price" => 'Compared At Price',
            "weight" => [
                'computed' => 'weight'
            ],
        ]);
        ...
        $attributeMap->each(function ($shopifyKey, $bagistoKey) {
            ...
            if (!is_array($shopifyKey)) {
                $this->modelAttributeData[$bagistoKey] = $this->product[$shopifyKey];
            } else {
                ...
                switch ($method) {
                    ...
                    case 'computed':
                        switch ($property) {
                            case 'handle':
                                // $property = 'products/' . $this->product['Handle'] .'-'. $this->product['Sku '];
                                $property = 'products/' . $this->product['Handle'];
                                break;
                            case 'vendor':
                                $property = $this->getBagistoAttributeOptionValueByName('brand', $this->product['Brand'])->id;
                                break;
                            case 'color':
                                $key = $value = null;
                                $optionValues = collect();
                                if(strtolower($this->product['Option1 Name']) == 'color'){
                                    $key = strtolower($this->product['Option1 Name']);
                                    $value = $this->product['Option1 Value'];
                                }else if(strtolower($this->product['Option2 Name']) == 'color'){
                                    $key = strtolower($this->product['Option2 Name']);
                                    $value = $this->product['Option2 Value'];
                                }else{
                                    $property = null;
                                }
                                if($key){
                                    $attribute = $this->attributeRepository->findOneByField('code', $key);
                                    if ($attribute) {
                                        $option = $attribute->hasMany(AttributeOption::class, 'attribute_id', 'id')->where('swatch_value', $value);
                                        $property = $option->first()->id;
                                    }else{
                                        $property = null;
                                    }
                                }
                            ...
                            //     break;
                            case 'weight':
                                $weight = floatval($this->product['Weight']);
                                if ($weight == 0.0) {
                                    $property = 1.0;
                                } else {
                                    $property = $weight;
...
//                                $property = null; <--- Im a dumb-ass
                                ...
                                $optionValues = collect();
                                $key = $value = null;
                                if(strtolower($this->product['Option1 Name']) == 'size'){
                                    $key = strtolower($this->product['Option1 Name']);
                                    $value = $this->product['Option1 Value'];
                                }else if(strtolower($this->product['Option2 Name']) == 'size'){
                                    $key = strtolower($this->product['Option2 Name']);
                                    $value = $this->product['Option2 Value'];
                                }else{
                                    $property = null;
        ...
        // Log::debug($this->modelAttributeData);
    }
    ...
    private function validateProductAttributeFamilyAttributes()
    {
        ...
        if ($validator->fails()) {
            $this->deleteBaseProductIfExists();
            Log::error('ProductsSeeder data for ' . $this->product['Sku '] . ' did not meet attribute requirements:');
    ...
    {
        // Log::debug($this->attributeFamilyOptions);
    ...
    public function insertProductImages()
    {
        ...
        // Log::debug('image url: '.$image. '   location: ' . $localSrc);
    ...
    public function downloadShopifyImage($src, $localPath)
    {
        try{
            ...
            Log::debug($e." image src: ".$src." image location: ".$localPath);
        }
    ...
    private function insertProductInventories()
    {
        ...
        $inventories[$inventorySourceId] = $this->product['Klb stock'];
        //may need to change to this
        // $inventories[$inventorySourceId] = $this->product['Klb stock']+$this->product['Pub stock'];
    ...
    {
        $productType = strtolower($this->product['Type']);
