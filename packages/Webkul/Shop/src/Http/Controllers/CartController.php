...
class CartController extends Controller
{
    ...
    {
        Log::error('you made it to the buyNow function...');
        try {
    ...
    /**
    * Returns true if the string (haystack) starts with the specified needle.
    * @param  [type] $haystack [description]
    * @param  [type] $needle   [description]
    * @return [type]           [description]
    * @link https://stackoverflow.com/a/834355/1620794
    */
    ...
    public function add($id)
    {
        try {
            ...
            if ($result instanceof CartModel) {
                ...
                // This function also looks at the name of the submit button to
                // see if a buy now button was clicked.
                Log::debug('request: ', request()->all());
                Log::debug('request (submit): ' . request()->submit);
                Log::debug('request input: ', request()->input());
                if (request()->get('is_buy_now') || $this->startsWith(request()->submit, 'buy_now')) {
                    Event::dispatch('shop.item.buy-now', $id);
        ...
        try {
            if (strlen($couponCode)) {
                ...
                if (Cart::getCart()->coupon_code == $couponCode) {
                    //look for a coupon code name and get it's description from db table
                    $couponDescription = CartRule::where('name', $couponCode)->first()->description;
