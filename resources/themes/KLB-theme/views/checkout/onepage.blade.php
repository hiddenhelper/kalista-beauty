        ...
        <div class="container">
            <div id="checkout" class="checkout-process row offset-lg-1 col-lg-11 col-md-12">
                ...
                <div class="col-lg-4 col-md-12 offset-lg-1 order-summary-container top pt0">
                  ...
                  <!--  <li><a href="{{route('shop.author.aboutauthors')}}" style=" font-family: aria-current; "> Authors </a></li>
-->
                    <app></app>
                    <br>
                    <coupon-component
...
-->
                    ...
                    <p>Shipping may take longer than normal due to high demand and expanded safety measures in our distribution centers. Please check your tracking number for expected delivery dates.</p>
                </div>
        ...
        (() => {
            ...
            Vue.component('checkout', {
                ...
                methods: {
                    ...
                    //takes customer card information if payment method is sekure or mpay
                    //$cardinfo
                    placeOrder: function () {
                        //var owner = document.getElementById("owner").value;
                        var expir = document.getElementById("year").value;
                        var month = document.getElementById("months1").value;
                        var cvv = document.getElementById("cvv").value;
                        var cardNumber = document.getElementById("cardNumber").value;
                        var yearMonth = (expir + "-" + month);
                        var creditInfo = [cardNumber, yearMonth, cvv ];

                        this.creditcard = creditInfo;




                        //echo"<pre>".var_dump($creditInfo)."<pre>";
                        if (this.isPlaceOrderEnabled) {
                            this.disable_button = false;
                            this.isPlaceOrderEnabled = false;


                            /**if payment method is sekure or mpay
                                $paymentMethod;
                                $cardinfo
                                !!validate if cardinfo is correct!!
                            */
                            this.$root.showLoader();
                            //pass card info along with token to save-order route
                            //this.$http.post("{{ route('shop.checkout.save-order') }}", {'_token': "{{ csrf_token() }}",$paymentMethod, $creditInfo })
                            this.$http.post("{{ route('shop.checkout.save-order') }}", {'card':this.creditcard, 'payment': this.selected_payment_method, '_token': "{{ csrf_token() }}"})
