...
class ProductImage extends AbstractProduct
{
    ...
    public function getProductBaseImage($product)
    {
        ...
        if ($images && $images->count()) {
            ...
            // ];
            $images[] = [
                'small_image_url'    => url('storage' . $image[0]->path),
                'medium_image_url'   => url('storage' . $image[0]->path),
                'large_image_url'    => url('storage' . $image[0]->path),
                'original_image_url' => url('storage' . $image[0]->path),
            ];
        }
